...
const pingInterval = setInterval(pingSweep, PING_INTERVAL_MS);
  ...
  switch (msg.type) {
    ...
    case "create-room": {
      ...
      if (meta.roomId && meta.isHost) {
        ...
        if (oldRoom) {
          oldRoom.peers.forEach((peerWs) =>
            send(peerWs, { type: "host-left" })
          );
          rooms.delete(meta.roomId);
          console.log(
            `[ROOM] Destroyed previous room ${meta.roomId} (host re-created)`
          );
        }
      }
    ...
    case "join-room": {
      ...
      if (meta.roomId && !meta.isHost) {
        ...
        if (oldRoom) {
          ...
          console.log(
            `[ROOM] Peer ${meta.clientId} left previous room ${meta.roomId} (re-joined)`
          );
        }
      }
      ...
      console.log(
        `[ROOM] Peer ${meta.clientId} joined room ${roomId} (${room.peers.size} peer(s) total)`
      );
      break;
    }
...
module.exports = { server, wss, rooms, pingSweep };
