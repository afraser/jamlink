...
function waitFor(condition, timeout = 1000) {
  return new Promise((resolve, reject) => {
    ...
    (function check() {
      ...
      if (Date.now() > deadline) return reject(new Error('waitFor: condition not met within ' + timeout + 'ms'));
      setTimeout(check, 10);
    })();
  });
}
...
describe('create-room', () => {
  ...
  it('generates unique room IDs for concurrent hosts', async () => {
    const [h1, h2, h3] = await Promise.all([connect(port), connect(port), connect(port)]);
    send(h1, { type: 'create-room' });
    send(h2, { type: 'create-room' });
    send(h3, { type: 'create-room' });
    const [m1, m2, m3] = await Promise.all([nextMessage(h1), nextMessage(h2), nextMessage(h3)]);
  ...
  it('cleans up the first room when host calls create-room a second time', async () => {
    ...
    assert.ok(!rooms.has(firstRoomId), 'first room should have been deleted — not leaked');
  ...
  it('notifies peers in the abandoned first room when host creates a second room', async () => {
    ...
    send(host, { type: 'create-room' });
    const [hostLeftMsg] = await Promise.all([nextMessage(peer), nextMessage(host)]);
...
describe('join-room', () => {
  it('responds with room-joined and notifies the host', async () => {
    ...
    const [peerMsg, hostMsg] = await Promise.all([nextMessage(peer), nextMessage(host)]);
  ...
  it("removes peer from the first room when they join a second room", async () => {
    const [h1, h2, peer] = await Promise.all([connect(port), connect(port), connect(port)]);
...
describe('disconnect cleanup', () => {
  ...
  it('does not leave behind rooms when the last peer exits a room the host already left', async () => {
    ...
    assert.equal(rooms.size, 0, 'no rooms should remain after everyone has left');
  });
});
